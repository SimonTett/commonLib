...
class test_R_gev(unittest.TestCase):
    def test_gev_fit_new(self):
        ...
        pars,se,cov,nll,aic,ks = gev_r.gev_fit_new(df, verbose=False)
        ...
        wts[::2] = 2.0
        pars2,se2,cov2,nll2,aic2,ks2 = gev_r.gev_fit_new(df,weights=wts, verbose=False)
        pd_tests.assert_series_equal(pars.round(1), expected, atol=1e-1, check_like=True)
    ...
    def test_gev_xarray(self):
        ...
        initial = xarray.DataArray(np.array([0.1,10.0,5.0,1.0,0.5]),coords=dict(parameter=['shape','location','scale','Dlocation_time','Dscale_time']))
        fit = gev_r.xarray_gev(ds, ds.time, dim='time',initial=initial,verbose=True)
        x_tests.assert_allclose(fit['Parameters'], expect, rtol=1e-1)
